...
class SwinTransformer2D_Adapter_AVQA(nn.Module):
    ...
    def __init__(self, grounding_pretrained=None, pretrained=None, img_size=224, patch_size=[1, 4, 4], num_frames=10, in_chans=3,
        ...
        if self.ftmode == 'multimodal' or self.ftmode == 'fusion':
            ...
            '''
        # TODO newly added
        self.avqatask_fc_a1 = nn.Linear(128, 512) # comment todo whether it is required? pre-train ----> Dec 10
        self.avqatask_fc_a2 = nn.Linear(512, 512) # todo pre-train
        ...
        self.avqatask_fc_fusion = nn.Linear(1024, 512) # todo
        ...
        self.avqatask_linear11 = nn.Linear(512, 512) # todo
        self.avqatask_dropout1 = nn.Dropout(0.1) # todo
        self.avqatask_linear12 = nn.Linear(512, 512) # todo

        self.avqatask_linear21 = nn.Linear(512, 512) # todo
        self.avqatask_dropout2 = nn.Dropout(0.1) # todo
        self.avqatask_linear22 = nn.Linear(512, 512) # todo
        self.avqatask_norm1 = nn.LayerNorm(512) # todo
        self.avqatask_norm2 = nn.LayerNorm(512) # todo
        self.avqatask_dropout3 = nn.Dropout(0.1) # todo
        self.avqatask_dropout4 = nn.Dropout(0.1) # todo
        #self.avqatask_norm3 = nn.LayerNorm(512) # comment

        self.avqatask_attn_a = nn.MultiheadAttention(512, 4, dropout=0.1) # todo
        self.avqatask_attn_v = nn.MultiheadAttention(512, 4, dropout=0.1) # todo

        # question
        self.avqatask_question_encoder = QstEncoder(93, 512, 512, 1, 512) # todo

        self.avqatask_tanh = nn.Tanh() # todo
        #self.avqatask_dropout = nn.Dropout(0.5) # comment
        self.avqatask_fc_ans = nn.Linear(512, 42) # todo

        self.avqatask_avgpool = nn.AdaptiveAvgPool2d((1, 1)) # todo
        self.avqatask_fc_gl = nn.Linear(1024, 512) # todo pre-train

        # combine
        self.avqatask_fc1 = nn.Linear(1024, 512) # todo pre-train
        #self.avqatask_relu1 = nn.ReLU() # comment
        self.avqatask_fc2 = nn.Linear(512, 256) # todo pre-train
        #self.avqatask_relu2 = nn.ReLU() # comment
        self.avqatask_fc3 = nn.Linear(256, 128) # todo pre-train
        #self.avqatask_relu3 = nn.ReLU() # comment
        self.avqatask_fc4 = nn.Linear(128, 2) # todo pre-train
        #self.avqatask_relu4 = nn.ReLU() # comment

        self.avqatask_yb_fc_v = nn.Linear(1536, 512) # todo
        self.avqatask_yb_fc_a = nn.Linear(1536, 128) # todo ---> Dec 10
...
##################################################################################################
    ...
    def forward(self, a, v, v_nega, question, mode):
        if mode == 'videoonly':
            ...
            f_a = self.avqatask_yb_fc_a(f_a) # todo [bt, n, 1536] --> [bt, n, 512]
            ...
            ### -------> yb: cvpr use
            f_a = f_a.mean(dim=1) # todo [bt, 512]
            audio = rearrange(f_a, '(b t) c -> b t c', b=BS, t=TT) # todo [b, 10, 512]
            ### <-----
            ...
            audio_feat = F.relu(self.avqatask_fc_a1(audio)) # todo to see whether it is required? ----> Dec 10
            #print(audio.shape) #[b, t, 512]
            #audio_feat = F.relu(audio)  # todo [b, 10, 512] ----> Dec 10
            audio_feat = self.avqatask_fc_a2(audio_feat) # todo [b, 10, 512]
            audio_feat_pure = audio_feat # todo [b, 10, 512]
            B, T, C = audio_feat.size()  # [B, T, C]
            audio_feat = audio_feat.view(B * T, C)  # [B*T, C] todo [b*10, 512]

            ## visual posi [2*B*T, C, H, W]
            B, T, C, H, W = visual_posi.size() # todo [b, 10, 512, 7, 7]
            temp_visual = visual_posi.view(B * T, C, H, W)  # [B*T, C, H, W] todo [b*10, 512, 7, 7]
            v_feat = self.avqatask_avgpool(temp_visual)  # [B*T, C, 1, 1] # todo [b*10, 512, 1, 1]
            visual_feat_before_grounding_posi = v_feat.squeeze()  # [B*T, C] todo [bt, 512]

            (B, C, H, W) = temp_visual.size()
            v_feat = temp_visual.view(B, C, H * W)  # [B*T, C, HxW] todo [b*10, 512, 49]
            v_feat = v_feat.permute(0, 2, 1)  # [B, HxW, C] todo [b*10, 49, 512]
            visual_feat_posi = nn.functional.normalize(v_feat, dim=2)  # [B, HxW, C] todo [b*10, 49, 512]

            ## audio-visual grounding posi
            audio_feat_aa = audio_feat.unsqueeze(-1)  # [B*T, C, 1] todo [b*10, 512, 1]
            audio_feat_aa = nn.functional.normalize(audio_feat_aa, dim=1)  # [B*T, C, 1] todo [b*10, 512, 1]
            ...
            visual_feat_grd = torch.matmul(x2_p, visual_feat_posi) # todo [b*10, 1, 512]
            visual_feat_grd_after_grounding_posi = visual_feat_grd.squeeze()  # [B*T, C] todo [b*10, 512]

            visual_gl = torch.cat((visual_feat_before_grounding_posi, visual_feat_grd_after_grounding_posi), dim=-1) # todo [b*10, 1024]
            visual_feat_grd = self.avqatask_tanh(visual_gl)
            visual_feat_grd_posi = self.avqatask_fc_gl(visual_feat_grd)  # [B*T, C] todo [b*10, 512]

            feat = torch.cat((audio_feat, visual_feat_grd_posi), dim=-1)  # [B*T, C*2], [B*T, 1024] todo [b*10, 1024]
            ...
            audio_feat_be = audio_feat_pure.view(B, -1, 512) # todo [b, 10, 512]
