...
#import dataloader_new as dataloader
import models
#import model.Swin_AVQAModel as AVQAModel # todo our current version Dec 9
import model.Swin_AVQAModel_V1 as AVQAModel # todo our v1 version Dec 10
import numpy as np
import warnings
import json
from sklearn import metrics
from traintest_adapt_avqa import train, validate # todo our final setting
...
parser.add_argument("--dataset", type=str, default="audioset", help="the dataset used", choices=["audioset", "esc50", "speechcommands", "fsd50k", "vggsound", "epic", "k400", 'ks-32', 'ucf101', 'hmdb51', 'ave-29', 'avsbench', 'music-avqa'])
...
parser.add_argument("--metrics", type=str, default="mAP", help="the main evaluation metrics in finetuning", choices=["mAP", "acc", "miou"])
parser.add_argument("--loss", type=str, default="BCE", help="the loss function for finetuning, depend on the task", choices=["BCE", "CE", "IoU"])
...
parser.add_argument("--dir_image", type=str, default='', help="training data json") # todo
parser.add_argument("--dir_audio_log_mel", type=str, default='', help="validation data json")
parser.add_argument("--dir_audio_wav", type=str, default='', help="training data json") # todo
parser.add_argument("--dir_mask", type=str, default='', help="validation data json")

# todo newly added for AVAQ
parser.add_argument("--grounding_pretrained", type=str, default='None', help="grounding pretrained model path") # todo
...
if args.bal == 'bal':
    ...
    train_loader = torch.utils.data.DataLoader(
        dataloader.AVQA_dataset(label=args.data_train, video_res14x14_dir=args.dir_image, audio_wave_dir=args.dir_audio_wav,
                                args=args, transform=transforms.Compose([ToTensor()]), mode_flag='train'),
...
# todo batch size as 1?
val_loader = torch.utils.data.DataLoader(
    dataloader.AVQA_dataset(label=args.data_val, video_res14x14_dir=args.dir_image, audio_wave_dir=args.dir_audio_wav,
                            args=args, transform=transforms.Compose([ToTensor()]), mode_flag='val'),
    batch_size=1, shuffle=False, num_workers=args.num_workers, pin_memory=True) # todo original: drop_last=True; args.batch_size
...
if args.model == 'cav-mae-ft':
    ...
    audio_model = models.ViT_ImageNet(label_dim=args.n_class, depth=12, num_video_frames=10, pretrained=args.pretrain_path)
    ...
    audio_model = models.MM_DeiT_ImageNet(label_dim=args.n_class, audio_length=args.target_length, stride=16, depth=12, fusion_depth=0,
    ...
    audio_model = models.MM_ViT_Fusion_ImageNet(label_dim=args.n_class, audio_length=args.target_length, depth=12,
                                          fusion_depth=1, num_video_frames=10, pretrained=args.pretrain_path, mode=args.ftmode)
...
elif args.model == 'MM-Swin-AVQA-Large':

    print('finetune Swin Large modality-specific layers')

    audio_model = AVQAModel.SwinTransformer2D_Adapter_AVQA(
                                                   patch_size=[1, 4, 4],
                                                   img_size=224,
                                                   num_frames=10,
                                                   embed_dim=192,
                                                   ...
                                                   window_size=7,
                                                   pretrained=args.pretrain_path,
                                                   grounding_pretrained=args.grounding_pretrained,
                                                   ftmode=args.ftmode,
                                                   adapter_mlp_ratio=[0.5, 0.25, 0.125, 0.0625],
