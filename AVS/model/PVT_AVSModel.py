...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, channel=256, opt=None, config=None, vis_dim=[64, 128, 320, 512], tpavi_stages=[],
                 tpavi_vv_flag=False, tpavi_va_flag=True):
...
############################################################
        self.x1_linear = nn.Linear(192, 64) # todo input dimension 128
        self.x2_linear = nn.Linear(384, 128) # todo input dimension 256
        self.x3_linear = nn.Linear(768, 320) # todo input dimension 512
        self.x4_linear = nn.Linear(1536, 512) # todo input dimension 1024
        ...
        for idx_layer, my_blk in enumerate(self.swin.layers):
            if not isinstance(my_blk.downsample, nn.Identity): # todo
...
#####################################################
    ...
    def forward(self, x, audio_feature=None):
        ...
        x1 = multi_scale[0].view(multi_scale[0].size(0), 64, 64, -1) # todo token length = XX * XX
        x2 = multi_scale[1].view(multi_scale[1].size(0), 32, 32, -1) # todo
        x3 = multi_scale[2].view(multi_scale[2].size(0), 16, 16, -1) # todo
        x4 = multi_scale[3].view(multi_scale[3].size(0), 8, 8, -1) # todo
