...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_frames, num_heads, window_size=7, shift_size=0,
        ...
        self.t_attn = t_attn
        # todo Dec 25 for AVS ablation study
        '''
        if t_attn:
    ...
    def forward(self, x):
        ...
        if self.mode == 'fusion_adapt':
            ...
            if self.t_attn:
                v = rearrange(v, '(b t) n c -> (b n) t c', t=self.num_frames, n=L_v, c=C)
                res_temporal_v = self.attn(self.norm1(v), temporal=True)
                #res_temporal_v = self.T_Adapter(res_temporal_v) # todo Dec 25 for AVS ablation study (2) just add temporal attn without adapt
                v = v + self.drop_path(res_temporal_v)
                v = rearrange(v, '(b n) t c -> (b t) n c', t=self.num_frames, n=L_v, c=C)

                a = rearrange(a, '(b t) n c -> (b n) t c', t=self.num_frames, n=L_a, c=C)
                res_temporal_a = self.attn(self.norm1(a), temporal=True, signal='audio')
                #res_temporal_a = self.T_Adapter_Audio(res_temporal_a) # todo Dec 25 for AVS ablation study (2) just add temporal attn without adapt
                a = a + self.drop_path(res_temporal_a)
                a = rearrange(a, '(b n) t c -> (b t) n c', t=self.num_frames, n=L_a, c=C)
            #'''
            ...
            v = v + vn # todo Dec 25 for AVS ablation study
            a = a + an # todo Dec 25 for AVS ablation study
...
class SwinTransformer2D_Adapter_AVS_Without_Adapt(nn.Module):
    r""" Swin Transformer
...
'''
ftmode = 'fusion'
pretrained ='/nobackup1/kai/research/multimodal/AudioVisual/pretrained_model/Swin_ViT/swin_large_patch4_window7_224_22k.pth' # '/nobackup1/kai/research/multimodal/AudioVisual/pretrained_model/Swin_ViT/swin_base_patch4_window7_224_22k.pth'
model = SwinTransformer2D_Adapter_AVS(pretrained=pretrained, img_size=224, patch_size=[1, 4, 4], num_frames=5, in_chans=3,
                 embed_dim=192, depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48],
                 window_size=7, mlp_ratio=4., frozen_stages=-1, qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True, t_relative=True,
                 use_checkpoint=False, ftmode='fusion', adapter_mlp_ratio = [0.25, 0.25, 0.25, 0.25],
                 channel=256, opt=None, config=None, vis_dim=[64, 128, 320, 512],
                 tpavi_stages=[0, 1, 2, 3], tpavi_vv_flag=False, tpavi_va_flag=True)

video = torch.randn((2, 3, 5, 224, 224))
audio = torch.randn((2, 5, 224, 224))
