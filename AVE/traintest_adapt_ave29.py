...
def train(audio_model, train_loader, test_loader, args):
    ...
    for name, param in base_params:
        if 'adapter' not in name and 'temporal_embedding' not in name and 'ln_post' not in name and 'Adapter' not in name and 'my_tokens' not in name and 'gate_' not in name and 'ln_before' not in name and 'temporal_position_bias_table' not in name:
    ...
    optimizer = torch.optim.Adam([{'params': adapt_parameters, 'lr': args.lr}, {'params': mlp_params, 'lr': args.lr * args.head_lr}], weight_decay=5e-7, betas=(0.95, 0.999)) # weight_decay=5e-7
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (a_input, v_input, labels) in enumerate(train_loader):
            ...
            # TODO newly added
            labels = rearrange(labels, 'b t c -> (b t) c')
            ...
            if model_type == 'CLIP' or model_type == 'MM-CLIP-AVE-Base':
                with autocast(), torch.backends.cuda.sdp_kernel(enable_flash=False) as disable:
