...
class SwinTransformer2D_Adapter_New(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, label_dim, pretrained=None, img_size=224, patch_size=[1, 4, 4], num_frames=10, in_chans=3,
...
'''
ftmode = 'audioonly'
pretrained =None # '/nobackup1/kai/research/multimodal/AudioVisual/pretrained_model/Swin_ViT/swin_base_patch4_window7_224_22k.pth'
model = SwinTransformer2D_Adapter(label_dim=101, pretrained=pretrained, img_size=224, patch_size=[1, 4, 4], num_frames=10, in_chans=3,
                 embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32],
                 window_size=7, mlp_ratio=4., frozen_stages=-1, qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True, t_relative=True,
                 use_checkpoint=False, ftmode=ftmode)

video = torch.randn((1, 3, 10, 224, 224))
audio = torch.randn((1, 10, 224, 224))
